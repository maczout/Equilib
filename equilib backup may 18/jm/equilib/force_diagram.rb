  ...
  class ForceDiagram
    ...
    def build_loop(joint,anchor_force,next_vertex)
      ...
      sorted_forces = joint.get_forces.sort do |f1,f2|
        # Arrange forces in Cremona configuration according to orientation of anchor
        a1xz = f1.get_angle(@anchor.ci.transformation,JM::Equilib::Force::XZ_PLANE,joint)
        test = (a1xz <=> f2.get_angle(@anchor.ci.transformation,JM::Equilib::Force::XZ_PLANE,joint)) if (a1xz)
        a1yz = f1.get_angle(@anchor.ci.transformation,JM::Equilib::Force::YZ_PLANE,joint)
        test = (a1yz <=> f2.get_angle(@anchor.ci.transformation,JM::Equilib::Force::YZ_PLANE,joint)) if (test.to_f==0.0 && a1yz)
        a1xy = f1.get_angle(@anchor.ci.transformation,JM::Equilib::Force::XY_PLANE,joint)
        test = (a1xy <=> f2.get_angle(@anchor.ci.transformation,JM::Equilib::Force::XY_PLANE,joint)) if (test.to_f==0.0 && a1xy)
        test
      end
      if joint.uid=='Body'
        puts "Body sorting check"
        sorted_forces.each {|f| puts f }
      end
