  ...
  class Force 
    ...
    class << self
      ...
      def load(s,ci,vis_ci,vec_ci,dupvec_ci)
        Force.new(s,ci,vis_ci_vec_ci,dupvec_ci)
      end

      def create(s,sj,ej)
        ci = Force.create_ci(s,sj,ej)
        Force.new(s,ci)
      end
      def create_ci(s,sj,ej)
    ...
    def direction;              line_of_action[1];  end
    def get_angle(plane,j)
      v = line_of_action[1].to_p3d.project_to_plane(plane).to_v3d
      v.reverse! unless j==self.start_joint
      ang = v.angle_between([1,0,0])
      ang = (2*Math::PI - ang) if v.y<0
      ang
    end
    def length;                 carefully { @ci.transformation.scaleZ * Force.length };  end

    attr_accessor               :v # taken at start joint
    def get_v_at(j)
      return if @v.nil?
      j==start_joint ? @v.clone : @v.reverse
    end
    def set_v_at(j,v)
      return if @v.nil?
      raise "v must be parallel to loa" unless v.parallel? direction
      @v = (j==start_joint ? v : v.reverse)
    end

    def magnitude
      return if @v.nil?
      (@v.same_direction?(direction) ? @v.length_jm : -@v.length_jm) / @s.force_diagram.scale
    end
    ...
    def fix;                    self.status='fixed';  end
    def assumed?;               status=='assumed';  end   
    def assume;                 self.status='assumed';  end

    def reciprocal?;            (start_joint.loop.value_after(f) == end_joint.loop[f]);  end
    ...
    class Visualization 
      ...
      class << self
        # assumes external component definitions have same bounding box as internal
        def length;                             Sandbox.active_sandbox.model.definitions['Equilib_ForceInternalVisualization'].bounds.depth;  end
        def diameter;                           Sandbox.active_sandbox.model.definitions['Equilib_ForceInternalVisualization'].bounds.width;  end
      ...
      Visualization.definition_names                      = Hash.new {|h,k| h[k]=Hash.new(&h.default_proc) }
      Visualization.definition_names[true][0]             = 'Equilib_ForceInternalVisualization'
      Visualization.definition_names[false][-1]           = 'Equilib_ForceExternalPushVisualization'
      Visualization.definition_names[false][0]            = 'Equilib_ForceExternalPushVisualization'
      Visualization.definition_names[false][+1]           = 'Equilib_ForceExternalPullVisualization'     
      def get_definition;                                 @c.s.model.definitions[Visualization.definition_names[@c.internal?][@c.sign]];  end

      Visualization.diameter_scale                        = 0.005
      Visualization.min_diameter                          = 1.0
      Visualization.max_diameter                          = 10.0
      Visualization.saturation_force                      = Visualization.max_diameter/Visualization.diameter_scale
      def get_transformation
        # pull visualization away from joint visualization for cleaner appearance
        jov = @c.direction.clone
        jov.length = calculate_joint_offset_vector(@c.start_joint)
        sp = @c.start_joint.position + jov
        jov.length = calculate_joint_offset_vector(@c.end_joint)
        ep = @c.end_joint.position - jov
        location = Geom::Transformation.new(sp,@c.direction)
      ...
      def calculate_joint_offset_vector(j);               j.visualization.diameter/2 + 0.75*calculate_diameter;  end      
      ...
      Visualization.colours                               = Hash.new {|h,k| h[k]=Hash.new(&h.default_proc) }
      Visualization.colours['fixed']                      = {-1=>Sketchup::Color.new(0,103,255), 0=>Sketchup::Color.new(205,205,205), +1=>Sketchup::Color.new(255,0,52)} # need to differentiate by status so alphas render properly
      Visualization.colours['assumed']                    = {-1=>Sketchup::Color.new(0,101,255), 0=>Sketchup::Color.new(203,203,203), +1=>Sketchup::Color.new(255,0,50)}
      Visualization.colours.default                       = {-1=>Sketchup::Color.new(0,102,255), 0=>Sketchup::Color.new(204,204,204), +1=>Sketchup::Color.new(255,0,51)}
      def get_material;                                   highlighted? ? ComponentInstanceWrapper.highlight_colour : Visualization.colours[@c.status][@c.sign];  end

      Visualization.alphas                                = {'fixed'=>1.0, 'assumed'=>0.25}
      Visualization.alphas.default                        = 0.5
    ...
    class Vector 
      ...
      def get_transformation
        location = Geom::Transformation.new(@c.start_joint.loop[@c],@c.v)
        z_scale = @c.magnitude / Vector.length
        scaling = Geom::Transformation.scaling(1,1,z_scale)
        location*scaling
      end
    ...
    class DuplicateVector < Vector
      class << self
        def length;               Sandbox.active_sandbox.model.definitions['Equilib_ForceVectorDuplicate'].bounds.depth;  end
      end

      def get_definition;         @c.s.model.definitions['Equilib_ForceDuplicateVector'];  end
      def get_transformation
        location = Geom::Transformation.new(@c.end_joint.loop[@c],@c.v)
        z_scale = -@c.magnitude/Vector.length
        scaling = Geom::Transformation.scaling(1,1,z_scale)
        location*scaling
      end
